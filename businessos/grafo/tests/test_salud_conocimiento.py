...
def test_endpoint_salud_conocimiento():
    ...
    body = r.json()
    assert body["reglas_total"] == 98  # Fases A-D + LMV 225 + LFPPI + servicios legales transversal (LRArt5 + LFPIORPI) + comercio exterior MX (LAdua + LCE) + logistica MX (LCPAF + LAC) + T-MEC (Caps. 4, 5 y 7)
