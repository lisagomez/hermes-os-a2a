...
POST /validar {tenant_id, marca, conversacion, respuesta, conversacion_id?}
  → {aprobado, gates, motivo}
GET /health
"""
    ...
    async def registrar(self, fila: dict) -> None:
        ...
        try:
            if self._http is not None:
                r = await self._http.post(f"{self._url}/rest/v1/crm_supervision", headers=headers, json=fila)
            else:
                async with httpx.AsyncClient(timeout=TIMEOUT_S) as client:
                    r = await client.post(f"{self._url}/rest/v1/crm_supervision", headers=headers, json=fila)
...
def build_app(juez: JuezLLM | None = None, auditoria: Auditoria | None = None) -> Starlette:
