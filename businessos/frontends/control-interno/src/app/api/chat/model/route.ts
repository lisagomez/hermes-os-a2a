...
// GET /api/chat/model — la verdad del modelo activo del agente (para el header al
// montar). Proxy auth-gated al daemon: { model, override, displayName }.
export async function GET() {
  ...
  try {
    // Timeout generoso: la primera llamada tras un restart del daemon puede
    // resolver el default del sistema con una sonda al SDK (~2-30s, luego cache).
    const upstream = await fetch(`${CLAUDECLAW_URL}/model`, {
