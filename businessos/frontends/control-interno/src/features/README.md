...
- `auth/` — login/signup/reset con Supabase + protección de rutas
- `chat/` — chat con tu agente (streaming SSE via el daemon)
- `command-center/` — HUD de voz (Live): STT, TTS, wake word
- `tasks/` — kanban/board + list view + detalle de tareas
- `calendar/` — calendario con espejo de Google Calendar + capa de crons
- `draw/` + `draw3/` — canvas colaborativo (motor v3)
- `notifications/` — web push (VAPID) + campana
- `activity/` — monitor de sesiones del daemon
- `conversations/` — feed en vivo de conversaciones de agentes
- `cron/` — monitor de jobs del scheduler del daemon
- `ops/` — stream de eventos operativos
- `search/` — Cmd+K global
