...
| `SUPABASE_SERVICE_ROLE_KEY` | — | key | **Workspace CRM** (`/crm`, movido desde Mission Control 2026-08-08): lecturas de `leads` + vistas del embudo y la única escritura (mover de etapa), todo server-side. El CRM queda FUERA del seam `NEXT_PUBLIC_COPILOT_DATA` (real-source por diseño, con chip propio); sin la key, `/crm` muestra un aviso honesto en vez de datos fingidos. Usa `SUPABASE_URL` o la `NEXT_PUBLIC_SUPABASE_URL` de auth |
