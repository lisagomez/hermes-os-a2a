...
export function construirUsuarioBloque(bloque: BloqueLLM, c: ContextoAnalisis): string {
  const partes = [
    ...
    '',
    `INTAKE DEL LEAD: empresa/giro: ${c.intake.giro} · tamaño: ${c.intake.tamano} · país: ${c.intake.pais} · web: ${c.intake.web || 'no proporcionada'} · linkedin del contacto: ${c.intake.linkedin || 'no proporcionado'} · notas: ${c.intake.notas || '—'}`,
  ]
