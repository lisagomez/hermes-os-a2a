...
const INTAKE_VACIO: IntakeLead = { telefono: '', email: '', linkedin: '', web: '', tamano: '11-50', modeloNegocio: '', giro: '', pais: 'México (MX)', direccion: '', notas: '' }

// ISO 3166-1 alfa-2: los 249 códigos oficiales; nombres en español vía Intl.
// El valor guardado es "Nombre (CÓDIGO)" — legible en prosa y con el identificador estándar.
const CODIGOS_ISO_3166_1 =
...
const PAISES = CODIGOS_ISO_3166_1
  .map((codigo) => ({ codigo, etiqueta: `${nombresRegion.of(codigo) ?? codigo} (${codigo})` }))
      ...
      <Card className="space-y-3 p-5">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          ...
          <label className="block text-[12px] font-medium text-ink-secondary">
            ...
            <select
              value={intake.pais}
              onChange={(e) => setIntake({ ...intake, pais: e.target.value })}
              className="input mt-1"
              data-testid="intake-pais"
            >
              {PAISES.map((p) => (
                <option key={p.codigo} value={p.etiqueta}>{p.etiqueta}</option>
              ))}
            </select>
          </label>
