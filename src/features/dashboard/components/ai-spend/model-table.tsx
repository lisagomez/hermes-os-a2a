...
export function ModelTable({ filas }: { filas: GastoPorModelo[] }) {
  ...
  return (
    <table className="w-full text-sm">
      ...
      <tbody>
        {orden.map((f) => (
          <tr key={f.modelo} className="border-b border-line/60 last:border-0">
            <td className="py-2 pr-2 text-ink">{f.modelo}</td>
            <td className="py-2 pr-2 text-right tabular-nums text-ink-secondary">
