  ...
  if (tareas.length === 0) {
    return (
      <Card className="p-10 text-center">
        <p className="text-sm font-medium text-ink-secondary">Sin tareas todavía</p>
          ...
          {tareas.map((t) => (
            <tr key={t.task_id} className="border-b border-line/60 last:border-0">
              <td className="py-2 pl-4 pr-2 align-top">
                <code className="text-xs text-ink-secondary">{t.task_id}</code>
              </td>
              <td className="py-2 px-2 align-top text-ink">{t.objetivo}</td>
              <td className="py-2 px-2 align-top">
                <EstadoTareaBadge estado={t.estado} />
              </td>
              <td className="py-2 px-2 align-top text-right tabular-nums text-ink-secondary">
                {t.intentos}
              </td>
              <td className="py-2 pl-2 pr-4 align-top text-ink-muted">
