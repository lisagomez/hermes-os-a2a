...
export const TARGET: Record<EstadoAgente, (i: number, t: number) => ObjetivoParticula> = {
  ...
  verificar(i, tt) { const cols = 36, rows = 6, rowH = 15, gx = CX - cols * 3.5 + (i % cols) * 7, gy = CY - 52 + Math.floor(i / cols) * rowH, scanY = CY - 52 + ((tt * 1.2) % (rows * rowH)), done = gy < scanY, at = Math.abs(gy - scanY) < 7.5; return { x: gx, y: gy, col: at ? C.warn : (done ? C.ok : C.base), a: at ? 1 : (done ? .85 : .5) }; },
