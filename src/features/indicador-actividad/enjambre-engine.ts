...
export const TARGET: Record<EstadoAgente, (i: number, t: number) => ObjetivoParticula> = {
  pensar(i, tt) { const hs = (a: number) => { const x = Math.sin(a) * 43758.5453; return x - Math.floor(x); }, u1 = hs(i * .123 + 1) + 1e-6, u2 = hs(i * .577 + 2), g = Math.sqrt(-2 * Math.log(u1)), th = 6.283 * u2, bi = .5 + .5 * Math.sin(tt * .03 + i * .9), spr = 28 + 20 * bi, dx = Math.sin(tt * .024 + i * 1.7) * 5, dy = Math.cos(tt * .027 + i * 2.1) * 5, sk = Math.sin(i * 7.3 + tt * .06) > .985; return { x: CX + g * Math.cos(th) * spr * 1.15 + dx, y: CY + g * Math.sin(th) * spr * .72 + dy, col: sk ? C.hot : C.base, a: sk ? .95 : .4 + .3 * bi }; },  // blob de pensamiento GAUSSIANO (Box-Muller por hash de i): denso al centro, cae suave -> nube LLENA sin anillo ni hueco (no "agujero negro"). Respiracion DESINCRONIZADA por particula (fase i*.9) + microderiva = converge/diverge organico. Chispa rosa = idea que asoma. Ver cerebro F363
