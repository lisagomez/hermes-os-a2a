...
export const TARGET: Record<EstadoAgente, (i: number, t: number) => ObjetivoParticula> = {
  ...
  escribir(i, tt) { const S = WRITE_SLOTS, cur = (tt * 1.7) % (S.length + 46); if (i < S.length && i < cur) { const hd = i >= cur - 1.6; return { x: S[i].x, y: S[i].y, col: hd ? C.hot : C.base, a: hd ? 1 : .85 }; } return { x: CX + Math.sin(i * 12.9 + tt * .02) * 140, y: CY - 98, col: C.base, a: .26 }; },
  construir(i, tt) { const S = BUILD_SLOTS, bt = (tt * .6) % (S.length + 64); if (i < S.length && i < bt) { const fr = i >= bt - 4; return { x: S[i].x, y: S[i].y, col: fr ? C.hot : C.base, a: 1 }; } const s = i % 2 ? 1 : -1; return { x: CX + s * (150 + Math.sin(i * 3.1 + tt * .05) * 40), y: CY - 70 + Math.cos(i * 2.3 + tt * .04) * 34, col: C.base, a: .5 }; },
  ...
  buscar(i, tt) { const ph = ((tt * 1.3 + i * 3) % 130) / 130, ang = i * 2.399 + tt * .01, r = ph * 128, mk = i % 9 === 0; return { x: CX + Math.cos(ang) * r * 1.5, y: CY + Math.sin(ang) * r * .75, col: mk && ph > .7 ? C.hot : C.blue, a: (1 - ph) * .9 }; },
  ...
  verificar(i, tt) { const cols = 36, gx = CX - cols * 3.5 + (i % cols) * 7, gy = CY - 52 + Math.floor(i / cols) * 15, scan = CX - cols * 3.5 + ((tt * 1.9) % (cols * 7)), done = gx < scan, at = Math.abs(gx - scan) < 9; return { x: gx, y: gy, col: at ? C.warn : (done ? C.ok : C.base), a: at ? 1 : (done ? .85 : .5) }; },
  ...
  desplegar(i, tt) { const ph = ((tt * 1.1 + i * 4) % 150) / 150; if (ph < .7) { const e = ph / .7; return { x: CX + ((i % 7) - 3) * 8, y: lerp(STRIP_Y - 20, CY - 70, e * e), col: C.base, a: .5 + e * .5 }; } const e = (ph - .7) / .3, ang = i * 2.399; return { x: CX + Math.cos(ang) * e * 130, y: (CY - 70) - Math.abs(Math.sin(ang)) * e * 40, col: C.ok, a: 1 - e }; },
};
